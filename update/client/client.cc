...
void PrintfContacts(const Contacts& contacts) { 
    for (int i = 0; i < contacts.contacts_size(); ++i) { 
        ...
        for (const PeopleInfo_Phone& phone : people.phone()) {
            cout << "Phone number" << j++ << ": " << phone.number() << endl;
        }
        ...
        for (int j = 0; j < set.field_count(); j++) {
            ...
            cout << "Unkown Field" << j+1 << ": "
                 << "  number: " << unknown_field.number();
            switch(unknown_field.type()) {
                case UnknownField::Type::TYPE_VARINT:
                    cout << "  Value: " << unknown_field.varint() << endl;
                    break;
                case UnknownField::Type::TYPE_LENGTH_DELIMITED:
                    cout << "  Value: " << unknown_field.length_delimited() << endl;
                    break;
                // case ...
            }
        }
